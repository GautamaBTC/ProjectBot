...
function Carousel({ index, onClose }: { index: number | null; onClose: () => void }) {
  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-[100] flex flex-col items-center justify-center px-4 py-10" style={{ background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)' }} role="dialog" aria-modal="true">
      ...
      <button onClick={() => setPos((p) => (p - 1 + WORKS.length) % WORKS.length)} aria-label="Назад" className="absolute left-3 md:left-8 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full" style={{ border: '1px solid rgba(212,175,55,0.4)', color: '#d4af37', background: 'rgba(212,175,55,0.06)' }}>
      ...
      <img ref={imgRef} src={WORKS[pos]} alt={`Работа ${pos + 1}`} className="max-w-full max-h-[78vh] object-contain rounded-[14px]" style={{ boxShadow: '0 30px 80px -20px rgba(0,0,0,0.8), 0 0 40px rgba(212,175,55,0.10)', border: '1px solid rgba(212,175,55,0.2)' }} />

      <button onClick={() => setPos((p) => (p + 1) % WORKS.length)} aria-label="Вперёд" className="absolute right-3 md:right-8 top-1/2 -translate-y-1/2 w-12 h-12 flex items-center justify-center rounded-full" style={{ border: '1px solid rgba(212,175,55,0.4)', color: '#d4af37', background: 'rgba(212,175,55,0.06)' }}>
      ...
      <div className="mt-3 text-[0.7rem] tracking-[0.2em] uppercase" style={{ color: 'rgba(212,175,55,0.7)', fontFamily: "'Inter', sans-serif" }}>
      ...
      <div className="section-container">
        ...
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="hero-btn-glass hero-btn-gold"
            style={{ borderRadius: '9999px', padding: '0.8rem 2.4rem' }}
          >
            Смотреть все работы
          </button>
        </div>
